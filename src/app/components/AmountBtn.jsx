...
function AmountBtn({ onAmountChange, maxAmount }) {
  ...
  return (
    <div className={`flex flex-col relative ${amount === maxAmount ? "after:content-['Max_selected_tickets'] after:absolute after:-bottom-7 after:left-0 after:text-main-1 after:text-xs " : ""}`}>
      <div className="flex place-items-center gap-1">
        ...
        <button onClick={add} className={`${amount === maxAmount ? "bg-feedback-disabled-2 cursor-not-allowed border-feedback-disabled-1 hover:border-feedback-disabled-1 active:bg-feedback-disabled-2" : "bg-main-2"} relative font-rethink aspect-square  px-3 py-1 text-main-1 text-2xl border-solid border-main-1 border-2 after:bg-main-1 after:absolute after:top-0 after:left-0 after:-z-10 after:content-[''] after:w-full after:h-full after:ml-1 after:mt-1 hover:after:mt-0 hover:after:ml-0 after:transition-all after:duration-75 hover:outline-1 hover:border-accent-1 hover:text-main-1 hover:transition-all active:bg-accent-1`}>
          +
        </button>
      </div>
    </div>
