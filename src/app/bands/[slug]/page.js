...
async function Band({ params }) {
  ...
  const response = await fetch(`http://https://gabby-dull-drip.glitch.me/bands/${slug}`);
  const band = await response.json();

  const responseBands = await fetch(`http://https://gabby-dull-drip.glitch.me/bands`);
  ...
  const scheduleResponse = await fetch(`http://https://gabby-dull-drip.glitch.me/schedule`);
